...
+ $\textbf{Faithful}$: List of conditional independencies perfectly matches the list of d-separation relations of some DAG.
```{r knitr-faith, echo=FALSE, fig.show='hold', out.width='60%'}
knitr::include_graphics("./figures/faith.png")
```
...
+ $\textbf{Markov Equivalent}$: Let $G_1=(V,E_1), G2 =(V,E_2)$ be two DAGs containing the same set of nodes $V$. Then $G_1$ and $G_2$ are called Markov equivalent if for every three mutually disjoint subsets $A,B,C \subset V$, A and B are d-seperated by C in $G_1$ iff A and B are d-seperated by C in $G_2$.
    ...
    - Conversely, if given a conditional independence oracle, one can only determine a DAG up to its equivalence class.
```{r knitr-markovqq, echo=FALSE, fig.show='hold', out.width='95%'}
knitr::include_graphics("./figures/Markov_equivalent.png")
```

+ $\textbf{CPDAG}$: General representation of Markov equivalent class for DAG.

It's the graph that has the same links as the DAGs in the equivalence class and has oriented all and only the edges common to all of the DAGs in the equivalence class.

```{r knitr-eg, echo=FALSE, fig.show='hold', out.width='80%'}
knitr::include_graphics("./figures/Markov_equivalent_2.png")
knitr::include_graphics("./figures/CPDAG2.png")
```


+ PC algorithm
```{r knitr-PC2, echo=FALSE, fig.show='hold', out.width='95%'}
knitr::include_graphics("./figures/PC_alg.png")
```

    - Builmann proved that PC algorithm is also consistent in high dimensional sparse settings.

Suppose we are looking at the correlation between $V_i$ and $V_j$ given the set of some other nodes $S$. Our hypothesis test is:
$$H_0: \rho_{ij|S} = 0\quad vs \quad H_1:\rho_{ij|S} \neq 0$$

It's performed based on Fisher's Z-transforma:
$$\hat{z}_{ij|S} = \frac{1}{2}log\big(\frac{1+\hat{\rho}_{ij|S}}{1-\hat{\rho}_{ij|S}}\big)$$

Under $H_0$, $\sqrt{n-|S|-3}\times\hat{z}_{ij|S} \sim N(0,1)$.

      + $\alpha$ is treated as tuning parameter in the algorithm.
      
      + In current implementations, the error rate of testing conditional independence is usually controlled individually for each test, without correcting the effect of multiple hypothesis testing. (Some more work is done regard of this.)
    

+ Score-based learning

+ GES algorithm $score_{BIC}(G:D) = l(\hat{\theta}_G:D) - \frac{log\  M}{2}Dim[G]$
...
Ancestral Graphs is proposed in 2002. (Richardson, Thomas; Spirtes, Peter (2002), "Ancestral graph Markov models", The Annals of Statistics, 30 (4): 962-1030, doi:10.1214/aos/1031689015). Here we briefly introduce some definitions to proceed along.
...
+ A vertex $\alpha$ is said to be an $\textbf{ancestor}$ of a vertex $\beta$ if either there is a directed path $\alpha \rightarrow ...\rightarrow \beta$ from $\alpha$ to $\beta$, or $\alpha = \beta$. A graph contains all three types of edges are called "mixed graph". 

+ A vertex $\alpha$ is said to be $\textbf{anterior}$ to a vertex $\beta$ if there is a path $\mu$ on which every edge is either of the form $\gamma - \delta$ or $\gamma \rightarrow \delta$ with $\delta$ between $\gamma$ and $\beta$, or $\alpha = \beta$; that is, there are no edges $\gamma \leftrightarrow \delta$ and there are no edges $\gamma \leftarrow \delta$ pointing toward $\alpha$. Such a path is said to be an anterior path from $\alpha$ to $\beta$.

+ An $\textbf{ancestral graph}$ G is a mixed graph in which the following conditions hold for all vertices $\alpha$ in G:
  
      +  $\alpha \notin ant(pa(\alpha) \cup sp(\alpha))$. [If $\alpha$ and $\beta$ are joined by an edge with an arrowhead at $\alpha$, then $\alpha$ is not anterior to $\beta$, .e. no directed loop]
      
      +  if $ne(\alpha) \neq \emptyset$ then $pa(\alpha) \cup sp(\alpha) = \emptyset$. [there be no arrowheads present at a vertex which is an endpoint of an undirected edge.]

      + Equivalent definition: (1) there are no directed cycles. (2) whenever ther is an edge $\alpha \leftrightarrow \beta$, there is no directed path from $\alpha$ to $\beta$ or from $\beta$ to $\alpha$.
      
```{r knitr-ans, echo=FALSE, fig.show='hold', out.width='95%'}
knitr::include_graphics("./figures/ancestral.png")
```  
  
    
  
+ Maximal ancestral graph (MAG) requires one more rule, every missing edge corresponds to a conditional independence. i.e., for every pair of non-adjacent nodes $\alpha, \beta$, there exists a set $\Gamma$ such that $\alpha \perp \!\!\! \perp \beta \ |\  \gamma$.
...
## Counterfactual (R package)
Reference: Chernozhukov, Victor, Iv?n Fern?ndez-Val, and Blaise Melly. "Inference on counterfactual distributions." Econometrica 81.6 (2013): 2205-2268.

+ General idea: 

    + 1. Non-parametric estimation of covariate distribution;
    
    + 2. Estimate the functionals of outcome given convariate for pre-specified model. (distribution functions, quantile functions, quantile effects, distribution effects, Lorenz curves, Gini coefficients)
    
    + 3. Construct confidence sets based on the variance from estimation.


+ Two conditions:

      - The first stage estimators of the conditional and covariate distributions to satisfy a functional central limit theorem. 
      
      - The second condition requires validity of the bootstrap for estimating the limit laws of the first stage estimators.
      
      - For causal interpretation, requires the exogenous variables independent.

+ "We consider simple counterfactual scenarios
consisting of marginal changes in the values of a given covariate, as well as more elaborate
counterfactual scenarios consisting of general changes in the covariate distribution or in the conditional
distribution of the outcome given covariates. For example, the changes in the covariate
and conditional distributions can correspond to known transformations of these distributions in
a population or to the distributions in different populations." 
       
       + change the marginal value or distribution of covariates related to the outcome variable of interest. Among woman, what is the wage if had higher education.
       
       + change the conditional distribution of the outcome given the covariates. The wage of a woman had it been a man.

+ Notations: different from the book.
 
Suppose we would like to analyze the wage differences between men and women. Let 0 denote the population of men and 1 the population of women. $Y_j$ denotes wages
and $X_j$ denotes job market-relevant characteristics affecting wages for populations $j = 0$ and $j = 1$. 

$F_{Y_0|X_0}(y|x),F_{Y_1|X_1}(y|x)$:  stochastic assignment of wages to workers with characteristics x, for men and women, respectively. 
        
$F_{Y<0|0>},F_{Y<1|1>}$: observed distribution function of wages for men and women.
        
$F_{Y<0|1>}$: counterfactual distribution function of wages that would have prevailed for women had they faced the men's wage schedule $F_{Y_0|X_0}$:
$$F_{Y<0|1>}(y):=\int_{\chi_1}{F_{Y_0|X_0}(y|x)dF_{X_1}(x)}$$

        - For this term to be well defined, the support of 1 needs to be contained in the support of 0. In the wage example, every female worker can be matched with a male worker with the same characteristics.
        
+ Decomposition:
$$F_{Y<1|1>} - F_{Y<0|0>} = [F_{Y<1|1>}-F_{Y<0|1>}]+[F_{Y<0|1>}-F_{Y<0|0>}]$$
where the first term in brackets is due to differences in the wage structure and the second term is a composition effect due to differences in characteristics. Also appliable to other functionals. (Transform between quantile and cdf).

+ These decomposition can be applied to distribution, quantile, Lorenzo curve, Gini coefficients etc. 
$$\Delta^{DE}(y)=F_{Y<j|k>}(y) - F_{Y<l|m>}(y)$$
$$\Delta^{QE}(y)=Q_{Y<j|k>}(\tau) - Q_{Y<l|m>}(\tau)$$


+ Focusing on quantile effects, there are three types of counterfactual effect can be isolated:
```{r knitr-ct, echo=FALSE, fig.show='hold', out.width='90%'}
knitr::include_graphics("./figures/counterfactype.png")
```

+ Regression model used: The analysis speciffically targets and covers the regression methods for estimating conditional distributions most commonly used in empirical work.
      
      + Classical regression: $Y=P(X)'\beta + Q_V(U)$.
      
      + Quantile regression: $F_{Y|X}(y|x)=\int_{(0,1)}1\{P(X)'\beta \leq y\}du$
      
      + Cox transformation model: $F_{Y|X}(y|x) = 1-exp(-exp(t(y)-P(x)'\beta))$
...
It is very common for big data sets to contain missing values. Some may be frequently absent for a given feature, or several features may be absent for specific observations. In either case, there are different ways to handle missing values. The use of each is specific to the null data you are working with. However you choose to handle the missing values, always note the process when publishing or submitting your design.

If a specific feature is missing a significant proportion of values, the general rule of thumb is to remove the observation from the model completely if 60% or more of the observations are missing values for that feature.

## Missing Data After Removing Useless Features

Many ways to handle unknown/missing values in your data:

- Removing entire observations
- Imputing values
    - Measures of central tendency
    - By correlations 
    - By similarities between cases

## Removing Observations

Once you have removed any unusable features, look for observations missing a value for the target feature. If we don't know their outcome, then the observation is useless and can immediately be thrown out.

If any single observation is missing many values (other than the target feature), it may or may not be a good idea to remove it. This is by far the simplest way to handle missing data, but removing it completely will decrease power of the analyses. It is generally best to use imputation if possible.

## Imputation

### Measures of Central Tendency

Although simple, this method of imputation can have serious ill effects if used incorrectly. For example, use of the *mean* for null values of a specific feature can alter the other central tendancy measures and decrease our variability. Of the three, the *mode* is the most commonly used, but only in the imputation of missing categorical values. If there is a clear mode, and a small number of null values, this is usually the best option. If there are only a few numerical values missing, then the *mean* is most acceptable, unless there are clear outliers. Be sure to check the distribution of that features values to determine skewness. If your data is not normal due to outliers, then the use of the *median* is typically best.

### Correlations

After you have removed all useless features and observations, and have used measures of centeral tendency where possible, have a look at your correlation table. If any two variables are highly correlated (typically .85 or above) it is common to use the correlation coefficient to imput the missing value of one of the variables.

### Similarities Between Cases

If correlation does not help with imputation, there exsist several functions based on the dataset as whole. Functions such as k-Nearest Neighbors (kNN) search the dataset for similar cases to those that are missing values. It will identify those closest across all factors, and imput the most likely value based on how far away you'd like it to look. kNN defaults to the closest 5 nearest observations. It will work for both categorical and numerical features.
